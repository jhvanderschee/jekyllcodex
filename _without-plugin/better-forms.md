...
It validates all fields with javascript on blur. It adds 'active', 'valid' and 'error' classes. This enables you to build all kind nice features. The 'active' class can be used to create Material Design styled labels that look like placeholders and shrink to make place for the cursor. The script places a serialized cookie on your computer with all fields. The cookies name equals the forms id. In the [form builder](/without-plugin/form-builder/){: .gray} the id of the form is generated and equal to the `page.slug`.
